...
<body class="flex flex-col h-screen overflow-hidden bg-gray-100">
  ...
  <div class="flex flex-col flex-1 overflow-hidden sm:flex-row">
    ...
    <main class="flex-1 overflow-y-auto p-4 sm:p-8 bg-[#999999]">
      ...
      <a href="{{ route('tirerequest.approval') }}" class="flex items-center px-4 py-2 mb-4 space-x-2 text-white bg-blue-600 rounded-lg hover:bg-blue-700 w-max">
      ...
      <div class="bg-white rounded-[30px] shadow p-4 sm:p-6 sm:px-16">
        ...
        <form class="space-y-4" method="POST" action="{{ route('tirerequest.sectionapprove', $request->id) }}">
          ...
          <div class="grid grid-cols-1 gap-4">
            ...
            <div class="flex flex-col space-y-2 md:flex-row md:items-start md:space-x-4 md:space-y-0">
              <label class="mt-1 text-sm font-medium md:w-1/3">Section Officer Comments</label>
              <textarea name="supervisor_comments" class="flex-1 px-4 py-2 bg-gray-200 border rounded-lg" rows="3">{{ $request->sectionApproval->supervisor_comments ?? '' }}</textarea>
            </div>
            <!-- Approve Service No -->
            <div class="flex flex-col space-y-2 md:flex-row md:items-center md:space-x-4 md:space-y-0">
              <label class="text-sm font-medium md:w-1/3">Approve Section Officer Service No</label>
              <input type="text" name="officer_services_number" class="flex-1 px-4 py-2 bg-gray-200 border rounded-full" value="{{ $request->sectionApproval->officer_services_number ?? '' }}" />
            </div>
