    ...
    <div class="flex-1 overflow-y-auto p-4 sm:p-8 bg-[#999999]">
      <div class="bg-white rounded-[30px] shadow p-4 sm:p-6">
        ...
        <div class="flex flex-col justify-between mb-4 md:flex-row md:items-center sm:mb-6">
          <h2 class="mb-4 text-xl font-extrabold text-blue-800 sm:text-2xl md:mb-0">Mechanic Approval Pending Requests</h2>
            ...
            <tbody id="tableBody">
              @forelse($requests->whereNull('mechanical_approval_status')->merge(
                ...
                <tr class="text-gray-700">
                  ...
                  <td class="px-4 py-2 border">
                    <a href="{{ route('transport.evaluationview', $req->id) }}" class="inline-block px-2 py-1 text-sm bg-green-800 rounded text-sky-50">View</a>
                  </td>
                </tr>
              @empty
                <tr>
                  <td colspan="8" class="py-4 text-center">No mechanic approval pending requests found.</td>
                </tr>
              @endforelse
            </tbody>
          </table>
