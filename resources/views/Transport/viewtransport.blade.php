    ...
    <div class="flex-1 overflow-y-auto p-4 sm:p-8 bg-[#999999]">
      <div class="bg-white rounded-[30px] shadow p-4 sm:p-6">
        ...
        <div class="flex flex-col justify-between mb-4 md:flex-row md:items-center sm:mb-6">
          <h2 class="mb-4 text-xl font-extrabold text-blue-800 sm:text-2xl md:mb-0">Transport Approval Pending Requests</h2>
                ...
                <tr class="text-gray-700">
                  ...
                  <td class="px-4 py-2 border">
                    @if($req->transport_approval_status === 'approved')
                      <span class="px-2 py-1 text-sm text-green-800 bg-green-200 rounded">Approved</span>
                    @elseif($req->transport_approval_status === 'rejected')
                      <span class="px-2 py-1 text-sm text-red-800 bg-red-200 rounded">Rejected</span>
                    @else
                  ...
                  <td class="px-4 py-2 border">
                    <a href="{{ route('transport.transportapprovalview', $req->id) }}" class="inline-block px-2 py-1 text-sm text-white bg-blue-700 rounded">View</a>
                  </td>
                </tr>
              @empty
                <tr>
                  <td colspan="8" class="py-4 text-center">No transport approval pending requests found.</td>
                </tr>
              @endforelse
            </tbody>
          </table>
