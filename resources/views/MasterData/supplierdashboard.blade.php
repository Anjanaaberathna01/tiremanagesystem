        ...
        <div class="flex flex-col justify-between mb-6 md:flex-row md:items-center">
          ...
          <div class="flex flex-col w-full gap-2 sm:flex-row sm:w-auto">
            <div class="relative w-full sm:w-80">
              <input id="supplierSearchInput"
                     onkeyup="filterTable('supplierTable', 'supplierSearchInput')"
                     placeholder="Search by any field..."
                     class="w-full py-2 pl-10 pr-4 border border-gray-300 rounded-full shadow-sm
            ...
            <button onclick="toggleForm()" type="button"
                    class="px-10 py-2 text-sm text-white bg-green-700 rounded-full shadow
                           hover:bg-green-800 sm:text-base">
              + Add Supplier
            </button>
          </div>
        </div>
        ...
        <div id="tireForm" class="hidden p-4 mb-6 bg-gray-100 border border-gray-300 rounded-xl">
          <form method="POST" action="{{ route('supplier.store') }}">
            ...
            <div class="flex justify-end mt-4">
              <button type="submit"
                      class="px-4 py-2 text-white bg-blue-700 rounded hover:bg-blue-800">Submit</button>
            </div>
          </form>
        </div>
